...
@Injectable()
export class QuestExistsMiddelware implements NestMiddleware {
    ...
    async use(req: Request, res: Response, next: NextFunction) {
        ...
        if (!postId) {
            throw new BadRequestException('QuestPost ID 파라미터는 필수입니다.');
        }

        const exists = await this.postService.hasExistedId(parseInt(postId));

        if (!exists) {
            throw new BadRequestException('QuestPost가 존재하지 않습니다.');
        }
